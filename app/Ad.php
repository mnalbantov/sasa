...
class Ad extends Model
{
    ...
    public static function getAds(array $request,array $address)
    {
        dump($address);
        $oLat = '';
        $oLng = '';
        ...
        if(isset($address['neighborhood']) && isset($address['city'])){
            $city = $address['city'];
            $neighborhood = $address['neighborhood'];
            $query->whereRaw("MATCH (address) AGAINST ('+$neighborhood, +$city' in boolean mode)");
        }
        if(!isset($address['neighborhood']) && isset($address['city'])) {
            $city = $address['city'];
            $query->whereRaw("MATCH (address) AGAINST ('+$city' in boolean mode)");
        }
        if(isset($address['neighborhood']) && !isset($address['city'])) {
            $neighborhood = $address['neighborhood'];
            $query->whereRaw("MATCH (address) AGAINST ('+$neighborhood' in boolean mode)");
        }
